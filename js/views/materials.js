 ...
 *
 * 접근 권한: HQ_ADMIN (업로드·삭제), INSTRUCTOR (조회·다운로드), SUPER_ADMIN (조회·삭제)
 * router allow: [ROLES.HQ_ADMIN, ROLES.INSTRUCTOR, ROLES.SUPER_ADMIN]
 *
 * 저장 경로: /materials/{materialId}
 * 교육 연결 준비: training 문서에 materialIds[] 배열 추가 가능 (추후 확장)
 */
...
/* ── 상태 ─────────────────────────────────────────────────── */
let state = { materials: [], loading: false };

/* ── 권한 헬퍼 ────────────────────────────────────────────── */
const canUpload = () => authStore.role === ROLES.HQ_ADMIN;
const canDelete = () => authStore.role === ROLES.HQ_ADMIN || authStore.role === ROLES.SUPER_ADMIN;
      ...
      <div class="card__body" style="padding:var(--space-4)">
        <div style="display:flex;gap:var(--space-3);flex-wrap:wrap">
          <div class="input-group" style="flex:2;min-width:200px">
            ...
            <input class="form-control" id="mat-search" type="search" placeholder="자료명으로 검색" />
          </div>
          <select class="form-control" id="mat-filter-type" style="flex:1;min-width:130px">
            <option value="">전체 유형</option>
            ${MATERIAL_TYPES.map(t => `<option value="${t}">${MATERIAL_TYPE_LABELS[t]}</option>`).join("")}
          </select>
        </div>
      </div>
    </div>
...
function renderTable(container) {
  ...
  const search = String(
    (container.querySelector("#mat-search") ?? document.getElementById("mat-search"))?.value ?? ""
  ).trim().toLowerCase();

  const typeFilter = String(
    (container.querySelector("#mat-filter-type") ?? document.getElementById("mat-filter-type"))?.value ?? ""
  );
    ...
    wrap.innerHTML = `
      <div class="empty-state" style="padding:var(--space-16)">
        <svg width="48" height="48" viewBox="0 0 48 48" fill="none" class="empty-state__icon">
          ...
          <path d="M20 4v8h12" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
          <path d="M12 18h16M12 23h10" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
        </svg>
        <div class="empty-state__title">등록된 교육자료가 없습니다.</div>
        ${canUpload() ? `<div class="empty-state__desc">PDF 업로드 버튼으로 자료를 추가하세요.</div>` : ""}
      </div>
    `;
    return;
  }
    ...
    <table class="data-table">
      ...
      <tbody>
        ${filtered.map(m => `
          <tr>
            <td>
              <div style="font-weight:var(--weight-semibold);color:var(--gray-800)">${esc(m.title)}</div>
              ${m.description ? `<div style="font-size:var(--text-xs);color:var(--gray-400);margin-top:2px">${esc(m.description)}</div>` : ""}
            </td>
            <td><span class="chip chip--info">${esc(m.typeLabel)}</span></td>
            <td class="cell--mono" style="font-size:var(--text-xs)">${esc(m.fileName ?? "-")}</td>
            ...
            <td class="cell--actions">
              <div style="display:flex;gap:4px;justify-content:flex-end">
                ...
                ` : `<span style="font-size:var(--text-xs);color:var(--gray-300)">URL 없음</span>`}
                ${canDelete() ? `
  ...
  wrap.querySelectorAll(".btn-mat-delete").forEach(btn => {
    btn.addEventListener("click", () => confirmDelete(btn.dataset.id, btn.dataset.title, container));
  });
}
...
function openUploadModal(container) {
  ...
  modal.open({
    ...
    body: `
      <div style="display:flex;flex-direction:column;gap:var(--space-5)">
        ...
        <div class="form-group">
          ...
          <input class="form-control" id="mat-title" type="text" placeholder="예) 신규입사자 초기교육 자료" maxlength="100" />
        </div>
        ...
        <div class="form-group">
          ...
          <textarea class="form-control" id="mat-desc" rows="2"
            placeholder="자료에 대한 간단한 설명을 입력하세요." maxlength="300"
        ...
        <div class="form-group">
          ...
          ">
            <svg width="36" height="36" viewBox="0 0 36 36" fill="none"
                 style="color:var(--gray-300);margin:0 auto var(--space-3)">
              <path d="M18 6v16M10 14l8-8 8 8" stroke="currentColor" stroke-width="1.5"
                    stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M6 28h24" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
            </svg>
            <div id="mat-file-label" style="font-size:var(--text-sm);color:var(--gray-400)">
              클릭하거나 PDF 파일을 여기에 끌어다 놓으세요
            </div>
            <div style="font-size:var(--text-xs);color:var(--gray-300);margin-top:var(--space-1)">
              PDF 전용 · 최대 ${formatFileSize(MAX_FILE_SIZE)}
            </div>
          </div>
          <input type="file" id="mat-file-input" accept="${ALLOWED_EXT}" style="display:none" />
          <div id="mat-file-error" style="font-size:var(--text-xs);color:var(--color-danger);margin-top:var(--space-1);display:none"></div>
        </div>
    ...
    actions: [
      ...
      { label: "업로드", variant: "primary",   onClick: async () => {
        ...
        if (!selectedFile){ toast.error("PDF 파일을 선택해 주세요."); return; }
        ...
        try {
          await saveMaterial({ title, trainingType: type, description: desc }, selectedFile);
  ...
  requestAnimationFrame(() => {
    ...
    const applyFile = (file) => {
      ...
      if (err) {
        ...
        fileLabel.textContent = "클릭하거나 PDF 파일을 여기에 끌어다 놓으세요";
        dropzone.style.borderColor = "var(--color-danger)";
        return;
      }
      selectedFile = file;
      fileLabel.innerHTML = `
        <span style="color:var(--gray-700);font-weight:var(--weight-medium)">${esc(file.name)}</span>
        <span style="color:var(--gray-400);margin-left:8px">${formatFileSize(file.size)}</span>
      `;
...
function confirmDelete(id, title, container) {
  modal.open({
    ...
    body:  `<p style="font-size:var(--text-sm);color:var(--gray-600)">
              ...
              <span style="color:var(--color-danger)">이 작업은 되돌릴 수 없습니다.</span>
            </p>`,
    actions: [
      { label: "취소", variant: "secondary", onClick: () => modal.close() },
...
function errorState(err) {
  return `<div class="empty-state" style="padding:var(--space-16)">
    ...
    <div style="font-size:var(--text-xs);color:var(--gray-400);margin-top:var(--space-2)">${esc(err?.message ?? "알 수 없는 오류")}</div>
  </div>`;
}

function fmtDate(ts) {
  if (!ts) return "-";
  return new Date(Number(ts)).toLocaleDateString("ko-KR", { year: "numeric", month: "2-digit", day: "2-digit" });
}
